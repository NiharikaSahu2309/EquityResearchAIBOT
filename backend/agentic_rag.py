...
class FinancialAnalysisAgent:
    ...
    def __init__(self, rag_pipeline, groq_client):
    ...
    def process_query(self, user_query: str, context_data: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self._execute_plan(plan, user_query, context_data)
    ...
    def _execute_plan(self, plan: List[Dict], original_query: str, context_data: Dict = None) -> Dict[str, Any]:
        """
    ...
    def _synthesize_answer(self, query: str, plan: List[Dict], results: Dict) -> str:
        """
        Synthesize a concise, summarized final answer from all analysis steps
        """
        try:
            ...
            for step_num, step_result in results['steps'].items():
                if step_result['success']:
                    ...
                    if tool == 'search' and isinstance(result, dict):
                        if 'documents' in result and result['documents']:
                            # Extract most relevant findings
                            for doc in result['documents'][:3]:  # Top 3 most relevant
                                sources.add(doc.get('source', 'Unknown'))
                                search_results.append(doc.get('content', '')[:300])  # First 300 chars
                        if 'summary' in result:
                            search_results.append(result['summary'])
                    
                    elif tool == 'calculate' and isinstance(result, dict):
                        if 'calculations' in result:
                            calculations.update(result['calculations'])
                    
                    elif tool == 'analyze_trends' and isinstance(result, dict):
                        if 'summary' in result:
                            trends['analysis'] = result['summary']
                    
                    elif tool == 'risk_assessment' and isinstance(result, dict):
                        if 'risk_assessment' in result:
                            risks.update(result['risk_assessment'])
            
            # Create synthesized prompt for final answer
            synthesis_prompt = f"""
            Based on the following analysis results, provide a CONCISE, SUMMARIZED answer to: "{query}"
            
            Search Results from Documents: {' | '.join(search_results[:2]) if search_results else 'None'}
            
            Key Calculations: {calculations if calculations else 'None'}
            
            Trend Analysis: {trends.get('analysis', 'None')}
            
            Risk Assessment: {risks if risks else 'None'}
            
            Sources: {', '.join(list(sources)[:3]) if sources else 'None'}
            
            INSTRUCTIONS:
            1. Provide a SUMMARIZED answer (maximum 4-5 sentences)
            2. Focus on the MOST IMPORTANT findings only
            3. Use bullet points for key metrics/data
            4. Cite specific sources when mentioning data
            5. Start with the main conclusion/finding
            6. If no relevant data found, state this clearly
            
            Format:
            **Key Finding:** [Main insight]
            **Supporting Data:** [Key metrics/numbers]
            ...
            **Summary:** [2-3 sentence conclusion]
            ...
            response = self.groq_client.chat.completions.create(
                ...
                temperature=0.1,
                max_tokens=800  # Limit for concise response
            )
    ...
    def _create_search_summary(self, results: List, query: str) -> str:
        """Create a concise, informative summary of search results"""
        if not results:
            return "No relevant documents found for your query."
        
        # Identify most relevant documents
        high_relevance = [r for r in results if r['relevance'] > 0.5]
        top_sources = list(set([r['source'] for r in results[:3]]))
        
        # Extract key information from top results
        key_info = []
        for result in results[:2]:  # Top 2 results
            content_snippet = result['content'][:200]  # First 200 chars
            if result.get('key_metrics'):
                key_info.append(f"Key data from {result['source']}: {content_snippet}...")
        
        if high_relevance:
            summary = f"Found {len(high_relevance)} highly relevant documents in {', '.join(top_sources[:2])}."
            if key_info:
                summary += f" {' '.join(key_info[:1])}"  # Add one key insight
            return summary
        else:
            return f"Found {len(results)} documents with partial relevance. Main sources: {', '.join(top_sources[:2])}."
